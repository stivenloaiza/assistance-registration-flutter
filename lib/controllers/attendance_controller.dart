...
class AttendanceController implements AttendancePort {
  ...
  @override
  Future<List<AttendanceModel>> findAttendanceByDateRange(
      String range, String userId) async {
    return await _attendanceService.getAttendanceByDateRange(range, userId);
  }

  @override
  Future<List<AttendanceModel>> findByProperty(
      String property, String valueProperty) async {
    return await _attendanceService.getAttendanceByProperty(
  ...
  Future<BarChartWidget> buildAttendanceWidget(
    ...
    $groupId
    """);
    final groupAttendance = await findAttendanceByUserAndDateRange(
        userId, dateRange, groupId); // Filtramos por grupo
    final List<AttendanceModel> attendanceByGroup =
        await findByProperty("group", groupId); // Get all attendance by groupId
    attendanceByGroup.forEach((attendance) {
      print("Attendance: ${attendance.toString()}");
    });
    final List<ChartData> chartData =
        processAttendanceForWidget(groupAttendance);

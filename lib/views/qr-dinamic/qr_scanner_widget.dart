...
class _QRScannerScreenState extends State<QRScannerScreen> {
  ...
  bool _isProcessing = false; // Variable para asegurarnos de que solo procesemos un QR a la vez
          ...
          onDetect: (capture) async {
            ...
            for (final barcode in barcodes) {
              ...
              if (qrData != null) {
                ...
                _cameraController.stop(); // Detenemos la cámara después de un escaneo exitoso
  ...
  Future<void> _verifyOTPAndRegisterAttendance(String qrData, BuildContext context) async {
    try {
      // Decodificar los datos del QR
      final Map<String, dynamic> qrDecoded = Map<String, dynamic>.from(jsonDecode(qrData));
  ...
  Future<void> _registerAttendance(String userId) async {
    try {
      final String dispositiveId = "IdDispositive"; // Cambiar por el identificador real del dispositivo
      final String groupId = "IdGroup"; // Cambiar por el identificador real del grupo
      final String timeStamp = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now()); // Fecha y hora actuales

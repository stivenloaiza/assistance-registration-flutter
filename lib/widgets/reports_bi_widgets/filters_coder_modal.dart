...
class _FilterCoderState extends State<FilterCoderModal> {
  ...
  Future<List<ChartData>> processAttendanceForBarChart(
    ...
    return groupAttendance.entries.map((entry) {
      ...
      final double absentPercentage = totalRecords > 0
          ? ((attendance['absent'] ?? 0) / totalRecords) * 100
          : 0.0;
      final double onTimePercentage = totalRecords > 0
          ? ((attendance['onTime'] ?? 0) / totalRecords) * 100
          : 0.0;
      final double latePercentage = totalRecords > 0
  ...
  Future<List<Map<String, dynamic>>> fetchAttendanceByGroup(
  ...
  Future<void> loadAndProcessAttendance(String range) async {
    final attendanceList =
        await UtilApplication.attendanceFindByDateRangeInstance(
      _firestore,
      range,
      userId,
    );
    ...
    List<Map<String, dynamic>> attendanceData = attendanceList.map((e) {
      String date = (e.timeStamp as Timestamp)
          .toDate()
          .toIso8601String()
          .split('T')
          .first;
